        ...
        {upcomingEvents.length === 0 ? (
          ...
          upcomingEvents.map((event) => (
            <div
              className="event-box"
              key={event.id}
              style={{ borderLeft: `8px solid ${typeColors[event.typ] ?? '#616161'}` }}>
              <div className="event-title">{event.title}</div>
              <div className="event-info">{formatDate(event.start)} - {formatDate(event.end)}</div>
              ...
              {event.kommentar && <div className="event-comment">{event.kommentar}</div>}
            </div>
          ))
        )}
      </div>
        ...
        {sortedExams.length === 0 ? (
          ...
          sortedExams.map((exam) => (
            <div
              className="event-box"
              key={exam.id}
              style={{ borderLeft: `8px solid ${typeColors[exam.typ] ?? '#616161'}` }}>
              ...
              {exam.kommentar && <div className="event-comment">{exam.kommentar}</div>}
            </div>
          ))
        )}
      </div>
